workflow(name="crater-tests", max_parallel=1)

# Nodes represent coarse execution groups for affected-test scheduling.
node(id="layout", depends_on=[])
node(id="browser", depends_on=[])
node(id="fullstack", depends_on=["layout", "browser"])

# Paint / VRT suites
task(
  id="paint-vrt",
  node="layout",
  cmd=["pnpm", "exec", "playwright", "test", "tests/paint-vrt.test.ts"],
  srcs=[
    "layout/**",
    "painter/**",
    "renderer/**",
    "css/**",
    "dom/css/**",
    "tests/paint-vrt.test.ts",
    "tests/helpers/crater-vrt.ts",
    "tests/helpers/crater-bidi-page.ts",
    "tests/helpers/flaker-quarantine.ts",
    "real-world/**",
  ],
  trigger="auto",
)

task(
  id="paint-vrt-levels",
  node="layout",
  cmd=["pnpm", "exec", "playwright", "test", "tests/paint-vrt-levels.test.ts"],
  srcs=[
    "layout/**",
    "painter/**",
    "renderer/**",
    "css/**",
    "dom/css/**",
    "tests/paint-vrt-levels.test.ts",
    "tests/helpers/crater-vrt.ts",
    "tests/helpers/crater-bidi-page.ts",
  ],
  needs=["paint-vrt"],
  trigger="auto",
)

task(
  id="paint-vrt-responsive",
  node="layout",
  cmd=["pnpm", "exec", "playwright", "test", "tests/paint-vrt-responsive.test.ts"],
  srcs=[
    "layout/**",
    "painter/**",
    "renderer/**",
    "css/**",
    "dom/css/**",
    "tests/paint-vrt-responsive.test.ts",
    "tests/helpers/crater-vrt.ts",
    "tests/helpers/crater-bidi-page.ts",
  ],
  needs=["paint-vrt"],
  trigger="auto",
)

task(
  id="paint-vrt-svg-intrinsic",
  node="layout",
  cmd=["pnpm", "exec", "playwright", "test", "tests/paint-vrt-svg-intrinsic.test.ts"],
  srcs=[
    "layout/**",
    "painter/**",
    "renderer/**",
    "css/**",
    "dom/css/**",
    "tests/paint-vrt-svg-intrinsic.test.ts",
    "tests/helpers/crater-vrt.ts",
    "tests/helpers/crater-bidi-page.ts",
  ],
  needs=["paint-vrt"],
  trigger="auto",
)

task(
  id="paint-vrt-gradients",
  node="layout",
  cmd=["pnpm", "exec", "playwright", "test", "tests/paint-vrt-gradients.test.ts"],
  srcs=[
    "layout/**",
    "painter/**",
    "renderer/**",
    "css/**",
    "dom/css/**",
    "tests/paint-vrt-gradients.test.ts",
    "tests/helpers/crater-vrt.ts",
    "tests/helpers/crater-bidi-page.ts",
  ],
  needs=["paint-vrt"],
  trigger="auto",
)

task(
  id="wpt-vrt",
  node="layout",
  cmd=["pnpm", "exec", "playwright", "test", "tests/wpt-vrt.test.ts"],
  srcs=[
    "layout/**",
    "painter/**",
    "renderer/**",
    "css/**",
    "dom/css/**",
    "tests/helpers/**",
    "wpt-tests/**",
  ],
  needs=["paint-vrt"],
  trigger="auto",
)

# Browser protocol and adapter suites
task(
  id="auth-flow-via-bidi",
  node="browser",
  cmd=["pnpm", "exec", "playwright", "test", "tests/auth-flow-via-bidi.test.ts"],
  srcs=[
    "browser/shell/**",
    "browser/http/**",
    "http/**",
    "webdriver/webdriver/**",
    "scripts/fixtures/**",
    "tests/auth-flow-via-bidi.test.ts",
    "tests/helpers/crater-bidi.ts",
  ],
  needs=["playwright-adapter"],
  trigger="auto",
)

task(
  id="playwright-adapter",
  node="browser",
  cmd=["pnpm", "exec", "playwright", "test", "tests/playwright-adapter.test.ts"],
  srcs=["browser/**", "webdriver/**", "dom/**", "js/**", "renderer/**"],
  trigger="auto",
)

task(
  id="crater-playwright-adapter",
  node="browser",
  cmd=["pnpm", "exec", "playwright", "test", "tests/crater-playwright-adapter.test.ts"],
  srcs=["webdriver/playwright/**", "tests/helpers/**", "browser/**", "dom/**", "js/**"],
  needs=["playwright-adapter"],
  trigger="auto",
)

task(
  id="playwright-adapter-user-scenarios",
  node="browser",
  cmd=["pnpm", "exec", "playwright", "test", "tests/playwright-adapter-user-scenarios.test.ts"],
  srcs=["webdriver/playwright/**", "browser/**", "dom/**", "js/**"],
  needs=["playwright-adapter"],
  trigger="auto",
)

task(
  id="playwright-adapter-chromium-parity",
  node="browser",
  cmd=["pnpm", "exec", "playwright", "test", "tests/playwright-adapter-chromium-parity.test.ts"],
  srcs=["webdriver/playwright/**", "webdriver/webdriver/**", "browser/**", "dom/**", "js/**"],
  needs=["playwright-adapter"],
  trigger="auto",
)

task(
  id="bidi-e2e",
  node="browser",
  cmd=["pnpm", "exec", "playwright", "test", "tests/bidi-e2e.test.ts"],
  srcs=["browser/**", "webdriver/**"],
  trigger="auto",
)

task(
  id="preact-compat",
  node="browser",
  cmd=["pnpm", "exec", "playwright", "test", "tests/preact-compat.test.ts"],
  srcs=["browser/**", "webdriver/**", "dom/**", "js/**"],
  needs=["playwright-adapter"],
  trigger="auto",
)

# Full-stack scenarios split by file or describe block.
task(
  id="website-loading",
  node="fullstack",
  cmd=[
    "pnpm",
    "exec",
    "playwright",
    "test",
    "tests/website-loading.test.ts",
    "--grep",
    "Website Loading Tests",
  ],
  srcs=["layout/**", "renderer/**", "css/**", "dom/css/**", "browser/**", "tests/helpers/**"],
  needs=["paint-vrt", "playwright-adapter"],
  trigger="auto",
)

task(
  id="script-execution-edge-cases",
  node="fullstack",
  cmd=[
    "pnpm",
    "exec",
    "playwright",
    "test",
    "tests/website-loading.test.ts",
    "--grep",
    "Script Execution Edge Cases",
  ],
  srcs=["browser/**", "webdriver/**", "dom/**", "js/**"],
  needs=["playwright-adapter"],
  trigger="auto",
)

task(
  id="browser-user-scenarios",
  node="fullstack",
  cmd=["pnpm", "exec", "playwright", "test", "tests/browser-user-scenarios.test.ts"],
  srcs=["layout/**", "renderer/**", "css/**", "dom/css/**", "browser/**", "tests/helpers/**"],
  needs=["paint-vrt", "playwright-adapter"],
  trigger="auto",
)

task(
  id="scroll-issue",
  node="fullstack",
  cmd=["pnpm", "exec", "playwright", "test", "tests/scroll-issue.test.ts"],
  srcs=["layout/**", "renderer/**", "css/**", "dom/css/**", "browser/**"],
  needs=["paint-vrt", "playwright-adapter"],
  trigger="auto",
)
