...
const CRATER_PLAYWRIGHT_API_SUPPORT_SOURCE = [
  {
    ...
    api: "newContext",
    status: "partial",
    notes: "Creates a lightweight Crater context wrapper until browser.close(); storageState, viewport, userAgent, locale, offline, geolocation, and permissions options have partial support, while most Playwright BrowserContext options are not implemented.",
  },
  {
    owner: "browser",
    api: "newPage",
  ...
  },
  {
    ...
    api: "newPage",
    status: "partial",
    notes: "Creates an isolated Crater tab over a hidden shared BiDi transport page and applies context cookies, current-origin localStorage, viewport, userAgent, locale, offline, geolocation, permissions, and routes; browser-grade storage/session isolation is limited.",
  },
  {
    owner: "context",
    api: "pages",
  ...
  },
  {
    ...
    api: "storageState",
    status: "partial",
    notes: "Snapshots visible cookies and localStorage into Playwright's { cookies, origins } shape and can write path files for newContext preload; sessionStorage is not implemented.",
  },
  {
    owner: "context",
    api: "cookies",
    status: "partial",
    notes: "Returns cookies stored through Crater's BiDi storage backend for open pages plus context-local pending cookies, with basic URL filtering.",
  },
  {
    owner: "context",
    api: "addCookies",
    status: "partial",
    notes: "Adds Playwright-shaped cookies to existing pages through Crater's BiDi storage backend and applies them to later pages; url-derived domain/path/secure defaults are supported for http(s) URLs.",
  },
  {
    owner: "context",
    api: "clearCookies",
    status: "partial",
    notes: "Clears context-local pending cookies and BiDi storage cookies for currently open pages, with basic name/domain/path string or RegExp filters.",
  },
  {
    owner: "context",
    api: "addInitScript",
    status: "partial",
    notes: "Adds a script to existing and later Crater pages by delegating to page.addInitScript(); full Playwright ordering across multiple context/page script sources is not implemented.",
  },
  {
    owner: "context",
    api: "setDefaultTimeout",
    status: "partial",
    notes: "Propagates the default timeout to existing and later Crater pages for polling-based waits; Playwright navigation-timeout separation is not implemented.",
  },
  {
    owner: "context",
    api: "setOffline",
    status: "partial",
    notes: "Applies Crater's BiDi network-conditions offline override to existing and later pages; detailed throttling and browser network stack parity are not implemented.",
  },
  {
    owner: "context",
    api: "grantPermissions",
    status: "partial",
    notes: "Applies Crater's BiDi permissions.setPermission override for geolocation and storage-access on existing and later pages; broad Playwright permission names and full origin wildcard semantics are not implemented.",
  },
  {
    owner: "context",
    api: "clearPermissions",
    status: "partial",
    notes: "Clears permission overrides previously applied through the context by resetting them to prompt; external browser permission state is not modeled.",
  },
  {
    owner: "context",
    api: "setGeolocation",
    status: "partial",
    notes: "Applies Crater's BiDi geolocation override to existing and later pages; requires a granted geolocation permission and does not emulate a native browser location provider.",
  },
  {
    owner: "context",
    api: "route",
    status: "partial",
    notes: "Registers a route handler on existing and later Crater pages by delegating to page.route(); concurrent routed pages, browser-grade context routing semantics, and service worker interception are not implemented.",
  },
  {
    owner: "context",
    api: "unroute",
    status: "partial",
    notes: "Removes delegated context route handlers from existing pages by matcher identity or clears all context routes; route-specific handler removal is not implemented.",
  },
  {
    owner: "context",
    api: "close",
