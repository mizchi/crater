...
- [ ] `painter/svg/types.mbt` を機能ごとに分割する
  ...
  - 進捗: `Color` constructors, `StrokeStyle::default`, `ViewBox::get_transform`, `Transform` operations, gradient `color_at`, `BoundingBox` / `ClipRect`, shape hit testing, `Camera` math, blend mode math, color filter math, mask math, pattern sampling は crater facade を維持しつつ `mizchi/svg` へ委譲
