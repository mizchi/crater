...
- `compat.css-values-baseline` を approved 化。`wpt.json` に css-values を modulePrefixes `["calc-"]` で登録し、`tests/wpt-baselines/css-values.env` に **34/46** を pin。calc サイジング系は 0.4.2 mixed calc (#67) + fixed-table 拡張 (#281) + abspos %height-vs-ICB 修正 (#282 cluster1 のうち calc-in-calc / parenthesis-stack) で回復。残 12 failure は table 余白分配 (#281) / max()・min() / calc-intrinsic / round() / ch-ex / gradient / transform-origin / vertical-align / min-* の別残差で **#282** に集約。
- **abspos 百分率高さ修正** (`layout/block`): 位置指定の祖先を持たない absolute 要素の包含ブロックは ICB（ビューポート）。静的親の auto(0) 高さではなく viewport 高さに対して % / top+bottom を解決するよう修正（CSS 2.1 §10.1）。layout 2435 + renderer 386 green。
