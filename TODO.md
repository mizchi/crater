...
- [ ] module boundary reset を段階的に進める
  ...
  - Refactor: `webdriver/webdriver` から以下を順に切り出す
    ...
    - `webdriver/rendering`: screenshot / print / actual paint / VRT bridge（param validation / actual paint pure helpers / captureScreenshotData actual-paint routing predicate / text metrics parser / glyph outline parser / batchRender input normalization / VRT result serialization は分離済み。次は JS provider / raster orchestration）
