...
- [x] nested element node construction を分離する
  - 完了: pre-computed selector を使う nested element の再帰 node 構築を `renderer/renderer/nested_element_node.mbt` へ移動し、`renderer.mbt` から実装責務を除去
...
- [ ] `painter/svg/types.mbt` を機能ごとに分割する
  ...
  - 進捗: gradient types / helpers を `painter/svg/gradient.mbt` へ分離
  ...
  - 進捗: tween / easing helpers を `painter/svg/tween.mbt` へ分離
  - 進捗: clip path helpers を `painter/svg/clip_path.mbt` へ分離
  - 進捗: mask helpers を `painter/svg/mask.mbt` へ分離
  - 進捗: pattern helpers を `painter/svg/pattern.mbt` へ分離
  - 進捗: marker / marked line helpers を `painter/svg/marker.mbt` へ分離
  - 進捗: use / symbol helpers を `painter/svg/use_symbol.mbt` へ分離
...
- [x] `painter/svg/scene.mbt` の scene graph mutation と shape rendering を分離する
  ...
  - 進捗: render context を `painter/svg/render_context.mbt` へ分離
  - 進捗: scene node factory を `painter/svg/scene_factory.mbt` へ分離
...
- [x] Linux 上で依存順 publish script を実行する manual workflow を追加した
  ...
  - `MOON_CREDENTIALS_JSON` secret を使って `dry-run` / `publish` を回す
...
### 直近の棚卸し
- `flaker` へ寄せ済み。`crater` 側は `@mizchi/flaker/reporting/*` wrapper にする
...
- `crater` の adapter として維持するもの
  ...
  - `scripts/flaker-batch-summary-core.ts` は `@mizchi/flaker` の generic aggregate に VRT/CSS 集約を重ねる overlay として維持する
...
- [x] quarantine contract を固定する
  - `taskId`, `spec`, `titlePattern`, `mode`, `scope`, `owner`, `reason`, `condition`, `expiresAt`
  - `@mizchi/flaker@0.12.5` の reporting export として contract / parser / match / expiry / summary / report を公開し、`crater` 側は wrapper 化した
...
- `ready-to-upstream`
  - なし。`scripts/flaker-upstream-inventory.ts` の exportable pure core は `@mizchi/flaker@0.12.5` までで upstream 済み
- `upstreamed`
...
- `keep-in-crater`
  - `flaker-batch-summary-core.ts` の VRT/CSS overlay
