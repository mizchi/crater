...
- run once to inspect current numbers:
  - `npm run bench:component-vrt`
- update the stored baseline after intentional improvements:
  - `npm run bench:component-vrt:update-baseline`
- verify against the stored baseline and absolute budget:
  - `npm run bench:component-vrt:check-baseline`
...
- override the sample count when needed:
  - `node scripts/component-vrt-bench-baseline.mjs check --warmup 2 --runs 7`

Stored baseline:

- `tests/component-vrt-bench-baseline.json`
...
- inspect current numbers:
  - `npm run bench:crater-cli`
- update the stored CLI baseline after intentional improvements:
  - `npm run bench:crater-cli:update-baseline`
- verify against the stored baseline and absolute budget:
  - `npm run bench:crater-cli:check-baseline`

Stored CLI baseline:

- `tests/crater-cli-bench-baseline.json`
