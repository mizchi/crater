     ...
     for (const eventType of _persistedListenerTypes) {
       ...
       for (const source of sources) {
         try {
           ...
           if (!store[eventType].includes(listener)) store[eventType].push(listener);
         } catch (e) {
 ...
 function addEventListenerImpl(target, type, listener) {
   if (listener === null || listener === undefined) return;
   const eventType = String(type);
   const store = getListenerStore(target);
   if (!store[eventType]) store[eventType] = [];
   if (!store[eventType].includes(listener)) store[eventType].push(listener);
   _persistEventListener(target, eventType, listener);
 }
 function removeEventListenerImpl(target, type, listener) {
   ...
   _removePersistedEventListener(target, eventType, listener);
 }
 function dispatchEventImpl(target, event) {
   ...
   const inlineHandler = target.getAttribute ? target.getAttribute(propName) : null;
   ...
   const propHandler = target[propName];
   if (typeof propHandler === 'function') {
     propHandler.call(target, event);
   }
   const listeners = target._listeners && target._listeners[eventType] ? target._listeners[eventType].slice() : [];
 ...
 Node.prototype.addEventListener = function(type, listener, options) { addEventListenerImpl(this, type, listener); };
 Node.prototype.removeEventListener = function(type, listener, options) { removeEventListenerImpl(this, type, listener); };
     ...
     for (const pseudo of pseudoMatches) {
       ...
       if (pname === 'invalid') {
         const isInvalidControl = (node) => {
           ...
           const tag = asciiLowercase(node._tagName || '');
           if (tag === 'select') {
             const options = node._children ? node._children.filter(n => n._nodeType === 1 && asciiLowercase(n._tagName || '') === 'option') : [];
             const selected = options.find(opt => opt.hasAttribute && opt.hasAttribute('selected')) || options[0] || null;
             if (!selected) return true;
             let selectedValue = selected.getAttribute ? selected.getAttribute('value') : null;
 ...
 function getReflectedStringAttribute(element, name) {
   const value = element && element.getAttribute ? element.getAttribute(name) : null;
   return value === null || value === undefined ? '' : String(value);
 }
 function setReflectedStringAttribute(element, name, value) {
   if (!element || !element.setAttribute) return;
   element.setAttribute(name, value === null || value === undefined ? '' : String(value));
 }
 function getReflectedBooleanAttribute(element, name) {
   return !!(element && element.hasAttribute && element.hasAttribute(name));
 }
 function setReflectedBooleanAttribute(element, name, value) {
   if (!element) return;
   if (value) {
 ...
 function getTextControlValue(element) {
   if (!element) return '';
   if (element.value !== undefined && element.value !== null) return String(element.value);
   if (element.textContent !== undefined && element.textContent !== null) return String(element.textContent);
   return '';
 }
 function clampSelectionIndex(element, value) {
   const length = getTextControlValue(element).length;
 ...
 function ensureSelectionState(element) {
   const value = getTextControlValue(element);
   const length = value.length;
   let start = element._selectionStart;
   let end = element._selectionEnd;
   if (start === undefined || start === null) start = length;
   if (end === undefined || end === null) end = start;
   start = clampSelectionIndex(element, start);
   end = clampSelectionIndex(element, end);
   if (end < start) end = start;
   element._selectionStart = start;
   element._selectionEnd = end;
   if (element._selectionDirection === undefined || element._selectionDirection === null) {
     element._selectionDirection = 'none';
   }
   return { start, end, direction: normalizeSelectionDirection(element._selectionDirection) };
 }
 function setSelectionState(element, start, end, direction) {
   const nextStart = clampSelectionIndex(element, start);
   const rawEnd = end === undefined || end === null ? nextStart : end;
   let nextEnd = clampSelectionIndex(element, rawEnd);
   if (nextEnd < nextStart) nextEnd = nextStart;
   element._selectionStart = nextStart;
   element._selectionEnd = nextEnd;
   element._selectionDirection = normalizeSelectionDirection(direction);
 }
 function preserveSelectionState(element, oldState, replaceStart, replaceEnd, insertedLength) {
   const replacedLength = Math.max(0, replaceEnd - replaceStart);
   const delta = insertedLength - replacedLength;
   let nextStart = oldState.start;
   let nextEnd = oldState.end;
   if (oldState.end <= replaceStart) {
     nextStart = oldState.start;
     nextEnd = oldState.end;
   } else if (oldState.start >= replaceEnd) {
     nextStart = oldState.start + delta;
     nextEnd = oldState.end + delta;
   } else {
     nextStart = Math.min(oldState.start, replaceStart);
     nextEnd = Math.max(nextStart, replaceStart + insertedLength);
   }
   setSelectionState(element, nextStart, nextEnd, oldState.direction);
 }
 Object.defineProperties(HTMLInputElement.prototype, {
   value: {
     get() { return getReflectedStringAttribute(this, 'value'); },
     set(v) { setReflectedStringAttribute(this, 'value', v); setSelectionState(this, getTextControlValue(this).length, getTextControlValue(this).length, 'none'); },
     configurable: true
   },
   checked: {
     get() { return getReflectedBooleanAttribute(this, 'checked'); },
     set(v) { setReflectedBooleanAttribute(this, 'checked', v); },
     configurable: true
   },
   selectionStart: {
     get() { return ensureSelectionState(this).start; },
     set(v) { const state = ensureSelectionState(this); setSelectionState(this, v, state.end, state.direction); },
     configurable: true
   },
   selectionEnd: {
     get() { return ensureSelectionState(this).end; },
     set(v) { const state = ensureSelectionState(this); setSelectionState(this, state.start, v, state.direction); },
     configurable: true
   },
   selectionDirection: {
     get() { return ensureSelectionState(this).direction; },
     set(v) { const state = ensureSelectionState(this); setSelectionState(this, state.start, state.end, v); },
     configurable: true
   }
 });
 HTMLInputElement.prototype.setSelectionRange = function(start, end, direction) {
   setSelectionState(this, start, end, direction);
 };
 HTMLInputElement.prototype.select = function() {
   setSelectionState(this, 0, getTextControlValue(this).length, 'none');
 };
 HTMLInputElement.prototype.setRangeText = function(replacement, start, end, selectionMode) {
   const value = getTextControlValue(this);
   const state = ensureSelectionState(this);
   const rangeStart = start === undefined ? state.start : start;
   const rangeEnd = end === undefined ? state.end : end;
   const nextStart = clampSelectionIndex(this, rangeStart);
   let nextEnd = clampSelectionIndex(this, rangeEnd);
   if (nextEnd < nextStart) nextEnd = nextStart;
   const text = replacement === null || replacement === undefined ? '' : String(replacement);
   setReflectedStringAttribute(this, 'value', value.slice(0, nextStart) + text + value.slice(nextEnd));
   const insertedEnd = nextStart + text.length;
   switch (selectionMode) {
     case 'select':
       setSelectionState(this, nextStart, insertedEnd, 'none');
       break;
     case 'start':
       setSelectionState(this, nextStart, nextStart, 'none');
       break;
     case 'preserve':
       preserveSelectionState(this, state, nextStart, nextEnd, text.length);
       break;
     case undefined:
     case null:
     case 'end':
     default:
       setSelectionState(this, insertedEnd, insertedEnd, 'none');
       break;
   }
 };
 Object.defineProperty(HTMLButtonElement.prototype, 'value', {
   get() { return getReflectedStringAttribute(this, 'value'); },
   set(v) { setReflectedStringAttribute(this, 'value', v); },
   configurable: true
 });
 Object.defineProperty(HTMLTextAreaElement.prototype, 'value', {
   get() { return this.textContent || ''; },
   set(v) { this.textContent = v === null || v === undefined ? '' : String(v); setSelectionState(this, getTextControlValue(this).length, getTextControlValue(this).length, 'none'); },
   configurable: true
 });
 ...
 HTMLTextAreaElement.prototype.setRangeText = function(replacement, start, end, selectionMode) {
   ...
   if (nextEnd < nextStart) nextEnd = nextStart;
   const text = replacement === null || replacement === undefined ? '' : String(replacement);
   this.textContent = value.slice(0, nextStart) + text + value.slice(nextEnd);
   const insertedEnd = nextStart + text.length;
   switch (selectionMode) {
     case 'select':
       setSelectionState(this, nextStart, insertedEnd, 'none');
 ...
 Object.defineProperties(HTMLOptionElement.prototype, {
   value: {
     get() {
       const attr = this.getAttribute ? this.getAttribute('value') : null;
       return attr === null || attr === undefined ? String(this.textContent || '') : String(attr);
     },
     set(v) { setReflectedStringAttribute(this, 'value', v); },
     configurable: true
   },
   selected: {
     get() { return getReflectedBooleanAttribute(this, 'selected'); },
     set(v) { setReflectedBooleanAttribute(this, 'selected', v); },
     configurable: true
   }
 });
 ...
 function ensureSelectSelection(select) {
 ...
 Object.defineProperties(HTMLSelectElement.prototype, {
   options: {
     get() {
       const items = getSelectOptions(this);
       return makeNodeList(() => items.slice());
     },
     configurable: true
   },
   selectedIndex: {
     get() {
       return ensureSelectSelection(this).selectedIndex;
     },
     set(v) {
       const state = ensureSelectSelection(this);
       const next = Number(v);
       for (let i = 0; i < state.options.length; i++) {
         state.options[i].selected = i === next;
       }
     },
     configurable: true
   },
   value: {
     ...
     set(v) {
       ...
       for (let i = 0; i < state.options.length; i++) {
         ...
         const isMatch = !matched && option.value === value;
 ...
 function createMockElement(tagName, mockId) {
   const el = {
     ...
     get ownerSVGElement() {
       if (this._namespaceURI !== 'http://www.w3.org/2000/svg') return null;
       let node = this._parent;
       while (node) {
         if (node._nodeType === 1 && node._namespaceURI === 'http://www.w3.org/2000/svg') {
           const local = asciiLowercase(node.localName || node._tagName || '');
           if (local === 'foreignobject') return null;
           if (local === 'svg') return node;
         }
         node = node._parent;
       }
       return null;
     ...
     setAttribute(name, value) {
       ...
       const existing = getAttrNodeByName(this, normalizedName);
       if (existing) {
         existing.value = strValue;
       } else {
         this._attrList.push(createAttrNode(
           this.ownerDocument || getGlobalDocument(),
           null,
           normalizedName,
           strValue,
           this,
     ...
     getAttribute(name) {
       const attr = getAttrNodeByName(this, name);
     ...
     removeAttribute(name) {
       ...
       const oldValue = this.getAttribute(searchName);
       const idx = this._attrList.findIndex(a => normalizeElementAttrLookupName(this, a.name) === searchName);
       if (idx >= 0) {
         this._attrList[idx].ownerElement = null;
     ...
     hasAttribute(name) {
       return getAttrNodeByName(this, name) !== null;
     },
     hasAttributes() { return this._attrList.length > 0; },
     getAttributeNames() { return this._attrList.map(a => a.name); },
     get attributes() {
       if (!this._namedNodeMap) {
         this._namedNodeMap = makeNamedNodeMap(this);
       }
       return this._namedNodeMap;
     },
     getAttributeNS(ns, localName) {
       const attr = getAttrNodeByNSLocalName(this, ns, localName);
       return attr ? attr.value : null;
     },
     setAttributeNS(ns, qualifiedName, value) {
       const normalNs = (ns === '' || ns === null || ns === undefined) ? null : ns;
       const parsed = validateAttributeQualifiedName(normalNs, qualifiedName);
       const localName = parsed.localName;
       const oldClassValue = (normalNs === null && localName === 'class') ? this.getAttribute('class') : null;
       const strValue = String(value);
       const oldValue = this.getAttributeNS(normalNs, localName);
       const existing = getAttrNodeByNSLocalName(this, normalNs, localName);
       if (existing) {
         existing.value = strValue;
       } else {
         this._attrList.push(createAttrNode(
     ...
     hasAttributeNS(ns, localName) {
       return getAttrNodeByNSLocalName(this, ns, localName) !== null;
     },
     ...
     removeAttributeNode(attr) {
       if (!attr || attr._nodeType !== 2 || attr.ownerElement !== this) {
         throw new DOMException('Attribute not found', 'NotFoundError');
       }
       if (attr.namespaceURI === null || attr.namespaceURI === '') {
         this.attributes.removeNamedItem(attr.name);
       } else {
         const idx = this._attrList.indexOf(attr);
         if (idx < 0) throw new DOMException('Attribute not found', 'NotFoundError');
         const oldClassValue = attr.localName === 'class' ? this.getAttribute('class') : null;
         this._attrList.splice(idx, 1);
         attr.ownerElement = null;
         if (oldClassValue !== null) updateClassIndex(this, oldClassValue, null);
       }
       return attr;
     },
     toggleAttribute(name, force) {
       if (!isValidAttributeName(String(name))) {
         throw new DOMException('Invalid character in attribute name', 'InvalidCharacterError');
       }
       const rawName = String(name);
       const isHtml = this._namespaceURI === 'http://www.w3.org/1999/xhtml';
     ...
     set textContent(v) {
       ...
       if (removedNodes.length > 0 || this._children.length > 0) {
         queueMutationRecord(this, {
           type: 'childList',
           addedNodes: this._children.slice(),
     ...
     appendChild(child) {
       ...
       const previousSibling = this._children.length > 0 ? this._children[this._children.length - 1] : null;
       ...
       if (String(this._tagName || '').toLowerCase() === 'script') maybeExecuteScriptElement(this);
       if (String(child._tagName || '').toLowerCase() === 'script') maybeExecuteScriptElement(child);
       return child;
     },
     insertBefore(newChild, refChild) {
       ...
       if (idx >= 0) {
         const previousSibling = idx > 0 ? this._children[idx - 1] : null;
         ...
         queueMutationRecord(this, {
           ...
           removedNodes: [],
           previousSibling: previousSibling,
           nextSibling: actualRef
         });
         if (String(this._tagName || '').toLowerCase() === 'script') maybeExecuteScriptElement(this);
         if (String(newChild._tagName || '').toLowerCase() === 'script') maybeExecuteScriptElement(newChild);
       } else {
         return this.appendChild(newChild);
       }
       return newChild;
     ...
     removeChild(child) {
       ...
       const previousSibling = idx > 0 ? this._children[idx - 1] : null;
       const nextSibling = idx + 1 < this._children.length ? this._children[idx + 1] : null;
       ...
       const cloneElement = (el, isDeep) => {
         ...
         for (const attr of el._attrList) {
           const clonedAttr = attr.cloneNode
             ? attr.cloneNode()
             : createAttrNode(
                 clone._ownerDocument || getGlobalDocument(),
                 attr.namespaceURI,
                 attr.name,
                 attr.value,
                 clone,
           ...
           clonedAttr.ownerDocument = clone._ownerDocument || getGlobalDocument();
           clonedAttr.ownerElement = clone;
           clone._attrList.push(clonedAttr);
     ...
     isEqualNode(other) {
       ...
       for (const a1 of attrs1) {
         const ns1 = (a1.namespaceURI === '' || a1.namespaceURI === undefined) ? null : a1.namespaceURI;
         const match = attrs2.find(a2 => {
           const ns2 = (a2.namespaceURI === '' || a2.namespaceURI === undefined) ? null : a2.namespaceURI;
     ...
     insertAdjacentElement(position, element) {
       if (!isNodeLike(element) || __craterGetNodeType(element) !== 1) {
     ...
     addEventListener(type, listener, options) { addEventListenerImpl(this, type, listener); },
     removeEventListener(type, listener, options) { removeEventListenerImpl(this, type, listener); },
 ...
 function createMockTextNode(text, mockId) {
   const node = {
     ...
     set textContent(v) {
       ...
       queueMutationRecord(this, { type: 'characterData', oldValue: oldValue });
     },
