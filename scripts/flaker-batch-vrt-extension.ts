import type {
  FlakerBatchSummary as BaseFlakerBatchSummary,
  FlakerBatchTaskSummary as BaseFlakerBatchTaskSummary,
...
export function applyFlakerBatchVrtExtension(
  baseSummary: BaseFlakerBatchSummary,
  ...
  const tasks: FlakerBatchTaskSummary[] = taskIds.map((taskId) => {
    ...
    const vrtSummary = vrtSummaries.get(taskId);
...
export function renderFlakerBatchVrtMarkdown(
