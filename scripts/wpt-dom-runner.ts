...
function createTestHarness() {
  return `
    ...
    function assert_throws_dom(name, func, description) {
      ...
      if (error.name !== name && error.code !== name) {
        throw new Error((description ? description + ': ' : '') +
    ...
    function __runTests() {
      for (const t of __tests) {
        try {
          if (t.async) {
            let completed = false;
            const doneFunc = t.func.call({
              step: function(f) { return f; },
            ...
            // For simple async tests, call done if returned
